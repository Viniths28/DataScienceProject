...
# Set page config
st.set_page_config(
    page_title="DataAnalytics Pro - Advanced Data Analytics, ML & Visualization Dashboard",
    page_icon="📊",
    layout="wide",
...
def main():
    st.markdown('<h1 class="main-header">📊 DataAnalytics Pro - Advanced Analytics, ML & Visualization</h1>', unsafe_allow_html=True)
    ...
    page = st.sidebar.selectbox(
        ...
        ["🏠 Home", "📊 Data Explorer", "📈 Data Analytics", "🤖 Machine Learning", "🎨 Beautiful Visualizations", "📋 Insights & Reports", "📋 Sample Datasets"]
    )
...
def show_machine_learning():
    st.markdown("## 🤖 Machine Learning Hub")
    st.markdown("### 🚀 Build and train powerful ML models with your data!")
    ...
    ml_type = st.selectbox(
        "🎯 Choose your ML adventure:",
        ["📈 Linear Regression", "🎯 K-Means Clustering", "📊 Logistic Regression"]
    )
    ...
    if ml_type == "📈 Linear Regression":
        ...
        show_logistic_regression(df)

def show_linear_regression(df):
    st.markdown("### 📈 Linear Regression Analysis")
    st.markdown("*Predict continuous values using linear relationships*")
    ...
    with col1:
        target_col = st.selectbox("🎯 Select target variable (what to predict):", numeric_cols)
    ...
    if st.button("🚀 Train Linear Regression Model", type="primary"):
        ...
        if len(X) < 10:
            st.error("Not enough data points for training!")
            return
...
def show_decision_trees(df):
    st.markdown("### 🌳 Decision Tree Analysis")
    st.markdown("*Build interpretable tree-based models for prediction*")
    
    numeric_cols = df.select_dtypes(include=[np.number]).columns
    categorical_cols = df.select_dtypes(include=['object']).columns
    
    if len(numeric_cols) < 2:
        st.warning("🤔 Need at least 2 numerical columns for decision tree analysis!")
        return
    ...
    if model_type == "Regression":
        ...
        if len(categorical_cols) == 0:
            ...
            return
        target_col = st.selectbox("🎯 Select target variable:", categorical_cols)
        feature_cols = st.multiselect(
            "📊 Select features:", 
...
def show_random_forest(df):
    ...
    st.markdown("*Ensemble learning with multiple decision trees for robust predictions*")
    
    numeric_cols = df.select_dtypes(include=[np.number]).columns
    categorical_cols = df.select_dtypes(include=['object']).columns
    
    if len(numeric_cols) < 2:
        st.warning("🤔 Need at least 2 numerical columns for Random Forest analysis!")
        return
    ...
    if model_type == "Regression":
        ...
        feature_cols = st.multiselect(
            "📊 Select features:", 
            [col for col in df.columns if col != target_col],
            default=[col for col in numeric_cols if col != target_col][:4]
        )
    else:  # Classification
        if len(categorical_cols) == 0:
            st.warning("No categorical columns found for classification!")
    ...
    if st.button("🚀 Train Random Forest Model", type="primary"):
        ...
        <div class="success-box">
            ...
            <p>Random Forest combines multiple decision trees to create a more robust and accurate model. Higher feature importance indicates variables that contribute more to predictions.</p>
        </div>
        """, unsafe_allow_html=True)

def show_kmeans_clustering(df):
    st.markdown("### 🎯 K-Means Clustering Analysis")
    st.markdown("*Discover hidden patterns and group similar data points*")
    ...
    if st.button("🚀 Perform K-Means Clustering", type="primary"):
        ...
        for i in range(n_clusters):
            ...
            for col in feature_cols:
                stats[f'Avg_{col}'] = f"{cluster_data[col].mean():.2f}"
        ...
        st.markdown("### 📈 Optimal Clusters Analysis")
        ...
        fig_elbow = px.line(
            ...
            title="📈 Elbow Method - Finding Optimal Clusters",
        ...
        <div class="success-box">
            ...
            <p>K-Means clustering groups similar data points together. The "elbow" in the chart above suggests the optimal number of clusters where adding more clusters doesn't significantly reduce inertia.</p>
        </div>
        """, unsafe_allow_html=True)

def show_logistic_regression(df):
    st.markdown("### 📊 Logistic Regression Analysis")
    st.markdown("*Predict categories and probabilities using logistic models*")
    
    numeric_cols = df.select_dtypes(include=[np.number]).columns
    categorical_cols = df.select_dtypes(include=['object']).columns
    
    if len(categorical_cols) == 0:
        st.warning("🤔 No categorical columns found for logistic regression!")
        return
    
    if len(numeric_cols) < 1:
        st.warning("🤔 Need at least 1 numerical column as features!")
        return
    ...
    with col1:
        target_col = st.selectbox("🎯 Select target variable (to predict):", categorical_cols)
    ...
    # Check number of unique values in target
    unique_targets = df[target_col].nunique()
    if unique_targets > 10:
        st.warning("Target variable has too many categories. Consider using a variable with fewer categories.")
        return
    ...
    if st.button("🚀 Train Logistic Regression Model", type="primary"):
        ...
        coef_df = pd.DataFrame({
            'Feature': feature_cols,
            'Coefficient': model.coef_[0] if len(model.coef_.shape) == 2 else model.coef_,
            'Abs_Coefficient': np.abs(model.coef_[0] if len(model.coef_.shape) == 2 else model.coef_)
        }).sort_values('Abs_Coefficient', ascending=False)
        ...
        <div class="success-box">
            ...
            <p>Logistic regression predicts the probability of categories. Positive coefficients increase the likelihood of the positive class, while negative coefficients decrease it.</p>
        </div>
        """, unsafe_allow_html=True)

def show_pca_analysis(df):
    st.markdown("### 🔬 Principal Component Analysis (PCA)")
    st.markdown("*Reduce dimensionality and discover the most important data patterns*")
    
    numeric_cols = df.select_dtypes(include=[np.number]).columns
    
    if len(numeric_cols) < 3:
        st.warning("🤔 Need at least 3 numerical columns for meaningful PCA analysis!")
        return
    ...
    max_components = min(len(feature_cols), len(df))
    n_components = st.slider("🎯 Number of components:", 2, min(10, max_components), 3)
    
    if st.button("🚀 Perform PCA Analysis", type="primary"):
        # Prepare data
        X = df[feature_cols].dropna()
        ...
        fig_pca = px.scatter(
            pca_df, x='PC1', y='PC2',
            title="🔬 Data in PCA Space (First 2 Components)",
        ...
        loadings_data = []
        for i in range(min(2, n_components)):  # Show first 2 components
        ...
        fig_loadings = px.bar(
            loadings_df, x='Loading', y='Feature', 
            color='Component',
        ...
        for i in range(n_components):
            summary_data.append({
                'Component': f'PC{i+1}',
                'Explained Variance': f'{pca.explained_variance_ratio_[i]:.3f}',
                'Cumulative Variance': f'{cumsum_var[i]:.3f}',
                'Eigenvalue': f'{pca.explained_variance_[i]:.3f}'
            })
        ...
        <div class="success-box">
            <h4>🔬 PCA Insights</h4>
            <p>PCA reduces data dimensionality while preserving most variance. Components with higher explained variance capture more important patterns in your data.</p>
        </div>
        """, unsafe_allow_html=True)
